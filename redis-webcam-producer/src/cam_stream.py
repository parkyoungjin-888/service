...
class CamStream:
    def __init__(self):
        ...
        self.logger = LoggerSingleton.get_logger(f'{app_config["name"]}.cam', level=log_level)
        ...
        while True:
            try:
                ...
                while True:
                    ...
                    if not ret:
                        self.logger.critical({'message': 'cap read is failed'})
                ...
                self.logger.error({'message': f'Error in stream: {e}'})
            finally:
                self.close()
    ...
    async def _send_stream(self):
        while True:
            try:
                ...
                self.logger.info({'message': 'data send success', 'send_count': len(data_ids)})
                SEND_COUNT.inc(len(data_ids))
                SEND_LATENCY.observe(time.time() - send_start)
            except Exception as e:
                self.logger.error({'message': f'Error in send_stream: {e}'})
            finally:
    ...
    async def _trim_stream(self):
        while True:
            try:
                ...
                if remove_count != 0:
                    self.logger.debug({'message': 'data send success', 'remove_count': remove_count})
                    REMOVE_COUNT.inc(remove_count)
                    TRIM_LATENCY.observe(time.time() - trim_start)
            except Exception as e:
                self.logger.error({'message': f'Error in trim_stream: {e}'})
            finally:
                await asyncio.sleep(1)
    ...
    def close(self):
        ...
        self.logger.info({'message': 'cam release is completed'})
